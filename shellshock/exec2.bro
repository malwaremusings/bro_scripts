...
event Exec2::file_line(description: Input::EventDescription, tpe: Input::Event, s: string)
	{
	print fmt("> Exec2::file_line(source=%s,name=%s,s=%s)",description$source,description$name,s);
...
event Input::end_of_data(name: string, source:string)
	{
	print fmt("> Input::end_of_data(name=%s,source=%s)",name,source);
	...
	local track_file = parts[2];
	print fmt("    track_file: %s",track_file);
	...
	if ( name !in pending_files )
		...
		{
		print fmt("    --- results[%s]$files[%s] ---",name,track_file);
		print results[name]$files[track_file];
		print fmt("    ---");
...
event InputRaw::process_finished(name: string, source:string, exit_code:count, signal_exit:bool)
	{
	print fmt("> InputRaw::process_finished(name=%s,source=%s)",name,source);
